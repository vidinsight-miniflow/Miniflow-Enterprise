...
from qbitra.domain.services import LoginService
...
async def authenticate_user(
    ...
    try:
        ...
        session_id = result_data.get("session_id")  # validate_access_token'dan session_id al
    ...
    request.state.session_id = session_id  # Session ID'yi request.state'e ekle
    
    # Trace context'e session_id ekle (middleware'den sonra çalıştığı için context'i güncelle)
    from qbitra.core.logger.context import get_current_context, set_current_context
    current_ctx = get_current_context()
    if current_ctx and session_id:
        # Mevcut context'i kopyala ve session_id ekle
        current_ctx.session_id = session_id
